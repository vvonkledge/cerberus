...
def main():
    try:
        ...
        try:
            ...
            # Optional HITL: when enabled, poll for dashboard decision
            if event_id and os.environ.get("SIANA_PRETOOLUSE_HITL") == "1":
                result = wait_for_response(
                    server_url=server_url,
                    event_id=event_id,
                    timeout=int(os.environ.get("SIANA_RESPONSE_TIMEOUT", "120")),
                    poll_interval=1.0,
                )
            ...
            pass  # Server integration must never block Claude Code
