"""Tests for the pre_compact hook's server integration."""
...
# Import the module under test (hooks/pre_compact.py)
import sys
...
class TestEventDataConstruction(unittest.TestCase):
    ...
    def _run_main_capture_post_event(self, input_data, env=None):
        ...
        with (
            ...
            patch("sys.argv", ["pre_compact.py"]),
            patch.dict(os.environ, env or {}, clear=False),
            patch("pre_compact.post_event", side_effect=fake_post_event),
            patch("pre_compact.log_pre_compact"),
        ):
            try:
                pre_compact.main()
            except SystemExit:
    ...
    def test_event_has_correct_hook_event_type(self):
        """Event data sets hook_event_type to PreCompact."""
        event_data = self._run_main_capture_post_event(
            {"session_id": "sess-1"}
        )
        self.assertEqual(event_data["hook_event_type"], "PreCompact")
    ...
    def test_event_has_payload(self):
        ...
        input_data = {"session_id": "sess-1", "trigger": "auto", "extra": "val"}
...
class TestServerUnreachable(unittest.TestCase):
    ...
    def test_exits_cleanly_when_server_unreachable(self):
        ...
        with (
            patch("sys.stdin", io.StringIO(stdin_data)),
            patch("sys.argv", ["pre_compact.py"]),
            patch("pre_compact.post_event", return_value=None),
            patch("pre_compact.log_pre_compact"),
        ):
            try:
                pre_compact.main()
    ...
    def test_local_log_still_written_when_server_down(self):
        ...
        input_data = {"session_id": "sess-1", "trigger": "manual"}
        stdin_data = json.dumps(input_data)
        ...
        with (
            ...
            patch("sys.argv", ["pre_compact.py"]),
            patch("pre_compact.post_event", return_value=None),
            patch("pre_compact.log_pre_compact") as mock_log,
        ):
            try:
                pre_compact.main()
            except SystemExit:
