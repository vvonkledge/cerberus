"""Tests for the stop hook's server integration."""
...
# Import the module under test (hooks/stop.py)
import sys
...
class TestEventDataConstruction(unittest.TestCase):
    ...
    def _run_main_capture_post_event(self, input_data, env=None):
        ...
        with (
            ...
            patch("stop.post_event", side_effect=fake_post_event),
            patch("stop.ensure_session_log_dir") as mock_dir,
        ):
    ...
    def test_event_has_correct_hook_event_type(self):
        """Event data sets hook_event_type to Stop."""
        event_data = self._run_main_capture_post_event(
            {"session_id": "sess-1"}
        )
        self.assertEqual(event_data["hook_event_type"], "Stop")
...
class TestServerUnreachable(unittest.TestCase):
    ...
    def test_exits_cleanly_when_server_unreachable(self):
        ...
        with (
            patch("sys.stdin", io.StringIO(stdin_data)),
            patch("sys.argv", ["stop.py"]),
            patch("stop.post_event", return_value=None),
            patch("stop.ensure_session_log_dir") as mock_dir,
        ):
            mock_dir.return_value = Path(tempfile.mkdtemp())
            try:
    ...
    def test_local_log_still_written_when_server_down(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            log_path = Path(tmpdir) / "stop.json"
