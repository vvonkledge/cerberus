"""Tests for the session_end hook's server integration."""
...
# Import the module under test (hooks/session_end.py)
import sys
...
class TestEventDataConstruction(unittest.TestCase):
    ...
    def _run_main_capture_post_event(self, input_data, env=None):
        ...
        with (
            ...
            patch("sys.argv", ["session_end.py"]),
            patch.dict(os.environ, env or {}, clear=False),
            patch("session_end.post_event", side_effect=fake_post_event),
            patch("session_end.log_session_end"),
        ):
            try:
                session_end.main()
            except SystemExit:
    ...
    def test_event_has_correct_hook_event_type(self):
        """Event data sets hook_event_type to SessionEnd."""
        event_data = self._run_main_capture_post_event(
            {"session_id": "sess-1"}
        )
        self.assertEqual(event_data["hook_event_type"], "SessionEnd")
    ...
    def test_event_has_payload(self):
        ...
        input_data = {"session_id": "sess-1", "reason": "clear", "extra": "val"}
...
class TestServerUnreachable(unittest.TestCase):
    ...
    def test_exits_cleanly_when_server_unreachable(self):
        ...
        with (
            patch("sys.stdin", io.StringIO(stdin_data)),
            patch("sys.argv", ["session_end.py"]),
            patch("session_end.post_event", return_value=None),
            patch("session_end.log_session_end"),
        ):
            try:
                session_end.main()
    ...
    def test_local_log_still_written_when_server_down(self):
        ...
        input_data = {"session_id": "sess-1", "reason": "clear"}
        stdin_data = json.dumps(input_data)
        ...
        with (
            ...
            patch("sys.argv", ["session_end.py"]),
            patch("session_end.post_event", return_value=None),
            patch("session_end.log_session_end") as mock_log,
        ):
            try:
                session_end.main()
            except SystemExit:
